import XCTest
@testable import Poollane

@MainActor
final class PoollaneTests: XCTestCase {
    ...
    func testAddEntryIncreasesCount() {
        ...
        store.add(PaymentEntry(title: "Test", vendor: "V", amount: 10, date: Date()))
    ...
    func testCannotAddMoreAtLimitWithoutPro() {
        ...
        while store.entries.count < Store.freeLimit {
            store.add(PaymentEntry(title: "Filler", vendor: "V", amount: 1, date: Date()))
        }
    ...
    func testDeleteEntryRemovesIt() {
        let store = Store()
        let entry = PaymentEntry(title: "ToDelete", vendor: "V", amount: 5, date: Date())
        store.add(entry)
        store.delete(entry)
    ...
    func testUpdateEntryChangesFields() {
        let store = Store()
        var entry = PaymentEntry(title: "Original", vendor: "V", amount: 5, date: Date())
        store.add(entry)
        entry.title = "Updated"
        store.update(entry)
